...
function SnackService.PlantSnack(player, plate, snackId)
	...
	local growTime = context.Config.GameConfig.DebugFastGrowth and context.Config.GameConfig.FastGrowthTime or (snack.GrowTime / math.max(0.1, context.Services.UpgradeService.GetMultiplier(player, "GrowSpeed")))
...
function SnackService.RestorePlanted(player)
	...
	for index = #getPlantedRecords(data), 1, -1 do
		...
		if not snack or not plate or getPlateNumber(plate) > context.Services.UpgradeService.GetPlateCount(player) then
			table.remove(data.PlantedSnacks, index)
		elseif not activeSnacks[record.UniqueId] then
			local elapsed = math.max(0, now - (tonumber(record.PlantedAt) or now))
			if offlineCap > 0 then
