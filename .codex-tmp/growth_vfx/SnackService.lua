...
local function worldTopY(instance)
	local boxCFrame, size = modelBoundingBox(instance)
	return boxCFrame.Position.Y + (size.Y * 0.5)
end
...
local function visualPlateTopY(plate)
	if typeof(plate) ~= "Instance" then
		return nil
	end
	local platesFolder = plate.Parent
	local plot = platesFolder and platesFolder.Parent
	local visuals = plot and plot:FindFirstChild("Visuals")
	if not visuals then
		return nil
	end
	...
	for _, child in ipairs(visuals:GetChildren()) do
		if child.Name:match("^GrowPlateVisual") then
			local visualPosition = targetPosition(child)
			if visualPosition then
				local dx = visualPosition.X - platePosition.X
				local dz = visualPosition.Z - platePosition.Z
	...
	if bestVisual and bestDistanceSq <= 64 then
		return worldTopY(bestVisual)
	end
	return nil
end

local function plateTopY(plate)
	local visualTop = visualPlateTopY(plate)
	if visualTop then
		return visualTop
	end
	if typeof(plate) == "Instance" and plate:IsA("BasePart") then
		return worldTopY(plate)
	end
