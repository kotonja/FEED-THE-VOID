...
local function modelBounds(model)
	...
	return ok and size or Vector3.new(2, 2, 2)
end
...
local function snackYOffsetFor(model)
	local size = modelBounds(model)
	return math.clamp((size.Y * 0.5) + 0.45, 0.95, 2.55)
end
...
local function targetSnackMaxSize(snackId, mutationId, stage, displayScale)
	...
	local cap = (displayScale or 1) < 1 and 4.15 or 4.45
...
function SnackService.PlantSnack(player, plate, snackId)
	...
	context.Services.AssetService.SetModelCFrame(model, CFrame.new(plate.Position + Vector3.new(0, snackYOffsetFor(model), 0)))
	...
	for uid, record in pairs(activeSnacks) do
		if record.Model and record.Model.Parent and record.Plate and record.Plate.Parent then
			...
			if stage ~= record.Stage then
				...
				SnackService.Context.Services.AssetService.SetModelCFrame(record.Model, CFrame.new(record.Plate.Position + Vector3.new(0, snackYOffsetFor(record.Model), 0)))
