...
function HealthCheckService.Run(player, reason)
	...
	if plots then
		for index = 1, 8 do
			...
			record(summary, plot ~= nil, "Plot" .. tostring(index), plot and "present" or "missing")
			if plot then
				record(summary, tonumber(plot:GetAttribute("PlotId")) == index, "Plot" .. tostring(index) .. ".PlotId", tostring(plot:GetAttribute("PlotId")))
				for _, childName in ipairs({ "PlotSpawn", "Plates", "SeedShopStation", "UpgradeStation", "SellStation", "DisplayShelf", "RebirthStation", "OwnerSign" }) do
					record(summary, plot:FindFirstChild(childName) ~= nil, "Plot" .. tostring(index) .. "." .. childName, plot:FindFirstChild(childName) and "present" or "missing")
				end
				local plates = plot:FindFirstChild("Plates")
				record(summary, childCount(plates) >= context.Config.GameConfig.MaxPlateCount, "Plot" .. tostring(index) .. ".Plates count", tostring(childCount(plates)) .. " plates")
				local rebirth = plot:FindFirstChild("RebirthStation")
				local rebirthPrompt = rebirth and (rebirth:FindFirstChild("RebirthPrompt") or rebirth:FindFirstChildWhichIsA("ProximityPrompt"))
				record(summary, rebirthPrompt ~= nil, "Plot" .. tostring(index) .. ".RebirthPrompt", rebirthPrompt and rebirthPrompt.Name or "missing")
			end
		end
	end
