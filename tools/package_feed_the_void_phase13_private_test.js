...
const outDir = path.join(root, "build", "feed_the_void_phase13_private_test");
const srcDir = path.join(outDir, "src");
const blueprintPath = path.join(outDir, "feed_the_void_phase13_private_test_overlay.blueprint.json");
const testingPath = path.join(outDir, "PHASE13_PRIVATE_TEST_QA.md");
...
function forceSourceForPhase13NewModules() {
  [
...
function writeTestingDoc() {
  fs.writeFileSync(testingPath, `# FEED THE VOID Phase 13 Private Test QA
...
addSoundGroups();
writeAllScripts();
forceSourceForPhase13NewModules();
addLaunchReadyUi();
...
const blueprint = {
  name: "FEED THE VOID Phase 13 Private Test Overlay",
  mode: "supervised",
  description: "Adds Phase 13 private-test polish: LaunchPageConfig, BugReportService snapshots, feedback improvements, screenshot spots, mix/VFX checks, private-test checklist, and mobile UI cleanup without rebuilding Workspace.GameWorld.",
  steps,
  metadata: {
    phase: "13-private-test",
