...
const outDir = path.join(root, "build", "feed_the_void_phase15");
const srcDir = path.join(outDir, "src");
const blueprintPath = path.join(outDir, "feed_the_void_phase15_private_test_qa_overlay.blueprint.json");
const testingPath = path.join(outDir, "PHASE15_PRIVATE_TEST_QA.md");
...
function writeTestingDoc() {
  fs.writeFileSync(testingPath, `# FEED THE VOID Phase 15 Private Test QA
...
const blueprint = {
  name: "FEED THE VOID Phase 15 Private Test QA Overlay",
  mode: "supervised",
  description: "Adds Phase 15 private-test QA tuning, debug report hardening, mobile action ownership fixes, and first-session balance while preserving imported assets and the user-made Workspace.GameWorld.",
  steps,
  metadata: {
