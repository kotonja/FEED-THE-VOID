...
const outDir = path.join(root, "build", "feed_the_void_phase14_imported_assets");
const srcDir = path.join(outDir, "src");
const blueprintPath = path.join(outDir, "feed_the_void_phase14_imported_assets_overlay.blueprint.json");
const testingPath = path.join(outDir, "PHASE14_IMPORTED_ASSETS_QA.md");
...
function writeTestingDoc() {
  fs.writeFileSync(testingPath, `# FEED THE VOID Phase 14 Imported Assets QA
...
const blueprint = {
  name: "FEED THE VOID Phase 14 Imported Assets Overlay",
  mode: "supervised",
  description: "Adds Phase 14 imported asset organization, complete AssetReferences, safe AssetService cloning/scaling, snack-specific visuals, event props, and asset debug commands without rebuilding Workspace.GameWorld.",
  steps,
  metadata: {
    phase: "14-imported-assets",
