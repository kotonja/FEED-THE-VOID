...
const outDir = path.join(root, "build", "feed_the_void_phase8_guidance");
const srcDir = path.join(outDir, "src");
const blueprintPath = path.join(outDir, "feed_the_void_phase8_guidance_overlay.blueprint.json");
const testingPath = path.join(outDir, "PHASE8_PRIVATE_TEST_QA.md");

const steps = [];
const includeScriptWrites = false;
...
function writeTestingDoc() {
  fs.writeFileSync(testingPath, `# FEED THE VOID Phase 8 Private Test QA
...
const blueprint = {
  name: "FEED THE VOID Phase 8 Guidance Overlay",
  mode: "supervised",
  description: "Adds Phase 8 guided private-test polish: client guidance, structured NextGoal, tutorial/failsafe recovery, LAB button, settings toggle, and health checks without rebuilding Workspace.GameWorld.",
  steps,
  metadata: {
    phase: "8-guided-private-test",
