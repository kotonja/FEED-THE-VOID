...
const outDir = path.join(root, "build", "feed_the_void_phase11_audio");
const srcDir = path.join(outDir, "src");
const blueprintPath = path.join(outDir, "feed_the_void_phase11_audio_overlay.blueprint.json");
const testingPath = path.join(outDir, "PHASE11_AUDIO_QA.md");
...
function writeScript(studioPath, className, relPath) {
  const properties = includeScriptWrites ? { Source: readSource(relPath) } : {};
...
function addLaunchReadyUi() {
  ...
  inst("TextLabel", "StarterGui.MainUI.PrivateTestWatermark", {
    ...
    Text: "PRIVATE TEST | v0.1.1-audio-private",
...
function addPrivateTestBoards() {
  ...
  const boards = [
    ...
    },
    {
      key: "ChangelogBoard",
      title: "BUILD v0.1.0",
      text: "Phase 11: full audio integration, quiet ambience, sound testing, and safe fallbacks.",
...
function writeTestingDoc() {
  fs.writeFileSync(testingPath, `# FEED THE VOID Phase 11 Audio QA
...
const blueprint = {
  name: "FEED THE VOID Phase 11 Audio Overlay",
  mode: "supervised",
  description: "Adds Phase 11 audio integration: SoundConfig, SoundController, AudioService, PlaySound remote, SoundGroups, UI/gameplay cues, ambience loops, debug sound commands, and audio health checks without rebuilding Workspace.GameWorld.",
  steps,
  metadata: {
