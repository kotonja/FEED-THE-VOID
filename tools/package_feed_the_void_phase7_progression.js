...
const outDir = path.join(root, "build", "feed_the_void_phase7_progression");
const srcDir = path.join(outDir, "src");
const blueprintPath = path.join(outDir, "feed_the_void_phase7_progression_overlay.blueprint.json");
const testingPath = path.join(outDir, "PHASE7_PROGRESSION_QA.md");
...
function writeTestingDoc() {
  fs.writeFileSync(testingPath, `# FEED THE VOID Phase 7 Progression QA
...
const blueprint = {
  name: "FEED THE VOID Phase 7 Progression Overlay",
  mode: "supervised",
  description: "Adds Phase 7 progression polish: snack expansion, rarity config, item locking, collection milestones, offline growth, shop unlocks, balance report, and QA hardening without rebuilding Workspace.GameWorld.",
  steps,
  metadata: {
