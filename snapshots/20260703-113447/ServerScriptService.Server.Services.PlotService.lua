...
local function removeOldBillboards(plot)
	local sign = plot and plot:FindFirstChild("OwnerSign")
	if not sign then
		return
	end
	for _, child in ipairs(sign:GetDescendants()) do
		if child:IsA("BillboardGui") and (child.Name == "OwnerBillboard" or child.Name == "OwnerLabelBillboard") then
			child:Destroy()
		end
	end
end
...
local function getLabel(plot)
	removeOldBillboards(plot)
	local signPart = getSignPart(plot)
	if not signPart then
		return nil
	end
	local gui = signPart:FindFirstChild("OwnerSurface")
	if not gui then
		gui = Instance.new("SurfaceGui")
		gui.Name = "OwnerSurface"
		gui.Face = Enum.NormalId.Front
		gui.CanvasSize = Vector2.new(420, 120)
	...
	if not label then
		...
		label.TextColor3 = Color3.fromRGB(244, 235, 255)
