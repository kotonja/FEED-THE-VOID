...
local function collectDisallowedUiText()
	local hits = {}
...
function BugReportService.PrintScreenshotSpots(player)
	...
	for _, spot in ipairs(spots) do
		...
		print("[FEED THE VOID][ScreenshotSpots] " .. spot.Name .. " position=" .. vecText(position) .. " lookAt=" .. vecText(spot:GetAttribute("LookAt")))
	end
	if player then
...
function BugReportService.MoveCamera(player, spotName)
	...
	BugReportService.Context.Services.VFXService.PlayForPlayer(player, "UI.CameraSpot", {
		...
		LookAt = spot:GetAttribute("LookAt"),
		NoThrottle = true,
	})
...
function BugReportService.PrivateTestCheck(player)
	...
	local mainUi = StarterGui:FindFirstChild("MainUI")
	local loading = mainUi and mainUi:FindFirstChild("LoadingPanel")
	local textHits = collectDisallowedUiText()
	...
	print("Ready: " .. ready)
	print("HealthCheck OK? " .. tostring(health and (health.Failed or 0) == 0 or false))
	print("SmokeTest OK? " .. tostring(smoke and (smoke.Failed or 0) == 0 or false))
	print("First10Check OK? report-only command available")
	...
	print("No stealing/trading text? " .. boolText(#textHits == 0))
