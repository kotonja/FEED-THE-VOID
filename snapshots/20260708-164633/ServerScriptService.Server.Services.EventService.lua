...
function EventService.StartEvent(eventName)
	...
	if context.Services.VFXService then
		...
		context.Services.VFXService.PlayForAll("Void.EventStart", {
			...
			Text = config.DisplayName or eventName,
			NoThrottle = true,
		})
	...
	if eventName == "SnackRain" then
		spawnEventVortex("SNACK RAIN", Color3.fromRGB(255, 180, 80), "EventSnackRainCloud")
	...
	elseif eventName == "VoidInfestation" then
		spawnEventVortex("VOIDMITES SWARM", Color3.fromRGB(175, 75, 255), "EventVoidmiteNest")
	...
	elseif eventName == "GoldenHunger" then
		spawnEventVortex("GOLDEN HUNGER", Color3.fromRGB(255, 215, 80), "EventGoldenHungerIdol")
	...
	elseif eventName == "MutationSurge" then
		spawnEventVortex("MUTATION SURGE", Color3.fromRGB(90, 255, 190), "EventMutationCrystal")
	...
	elseif eventName == "PhantomSnackChase" then
		spawnEventVortex("PHANTOM CHASE", Color3.fromRGB(172, 116, 255), "PhantomSnack")
