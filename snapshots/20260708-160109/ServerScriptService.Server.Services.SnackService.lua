...
local function replaceGrowingModel(record, stage)
	...
	local model = createSnackModel("Growing_" .. tostring(record.Plate:GetAttribute("SnackUid") or record.SnackId), oldPosition, record.SnackId, "Normal", stage, 1)
	...
	for uid, record in pairs(activeSnacks) do
		if record.Model and record.Model.Parent and record.Plate and record.Plate.Parent then
			...
			if stage ~= record.Stage then
				...
				fireEffect(record.Player, stage >= 3 and "GrowthReady" or "GrowthStage", model, growthEffectPayload(record.Plate, model, record.SnackId, stage))
