...
local function collectMobileUiWarnings(player)
	...
	if not nav then
		...
		local screenWidth = main.AbsoluteSize and main.AbsoluteSize.X or 0
		local phoneSized = screenWidth > 0 and screenWidth < 900
		if mobileAction and (mobileAction.Size.Y.Offset > 58 or (phoneSized and mobileAction.Position.X.Scale > 0.78)) then
			table.insert(warnings, "Contextual action button may overlap mobile controls")
		end
	end
	return warnings
end
